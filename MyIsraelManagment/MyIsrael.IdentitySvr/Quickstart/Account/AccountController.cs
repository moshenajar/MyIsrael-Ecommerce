...
using MyIsrael.IdentitySvr.UserServices;
using System;
using System.Linq;
...
namespace MyIsrael.IdentitySvr
{
  ...
  //  [SecurityHeaders]
    ...
    public class AccountController : Controller
    {
        ...
        private readonly IUserRepository _UsersFromDB;
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                //var sub = await _userManager.GetUserIdAsync(model.Username,mo)
                ...
                if (result.Succeeded)
                {
                    if (context != null)
                    {
                        if (await _clientStore.IsPkceClientAsync(context.ClientId))
                        {
                            // if the client is PKCE then we assume it's native, so this change in how to
                            // return the response is for better UX for the end user.
                            return View("Redirect", new RedirectViewModel { RedirectUrl = "https://www.google.com/"/*model.ReturnUrl*/ });
                        }
        ...
        public async Task<IActionResult> Registration([FromBody] UserRegistrationModel model)
        {
            ...
            //var result = await _userManager.CreateAsync(user, password: model.NewPassword);
